...
export function HistoryPage({ onBack }: { onBack: () => void }) {
  ...
  const historyLimit = useConfigStore((s) => s.config?.history_limit);
  ...
  // Esc returns to the format page, unless a dialog is open (the dialog handles
  // Escape to close itself).
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key !== "Escape") return;
      if (showConfirm || selectedItem) return;
      onBack();
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [showConfirm, selectedItem, onBack]);
...
function ConfirmDialog({
  ...
  const sheetRef = useModalA11y<HTMLDivElement>(onCancel);
  return (
      ...
      >
        <div className="modal-body">
          ...
          <div className="flex justify-end gap-2">
            <button onClick={onCancel} className="tool-button">
              {t("common.cancel")}
            </button>
            <button
              onClick={onConfirm}
