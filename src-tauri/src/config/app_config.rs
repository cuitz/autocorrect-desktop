...
pub enum LanguageMode {
    ...
    #[default]
    ZhCn,
    #[serde(rename = "en", alias = "En", alias = "en-US", alias = "en-us")]
    En,
}
...
pub struct FormatterRules {
    /// Auto add spacing between Chinese text and Latin letters or numbers.
    #[serde(default = "default_true")]
    pub space_word: bool,
    /// Add space around punctuation-like symbols near Chinese text.
    #[serde(default = "default_true")]
    pub space_punctuation: bool,
    /// Add space around brackets when near Chinese text.
    #[serde(default = "default_true")]
    pub space_bracket: bool,
    /// Add space around dash separators near Chinese text.
    #[serde(default = "default_true")]
    pub space_dash: bool,
    /// Add space around backticks near Chinese text.
    #[serde(default = "default_true")]
    pub space_backticks: bool,
    /// Add space around dollar markers near Chinese text.
    #[serde(default)]
    pub space_dollar: bool,
    /// Convert punctuation near Chinese text to fullwidth punctuation.
    #[serde(default = "default_true")]
    pub fullwidth: bool,
...
impl FormatterRules {
    pub fn autocorrect_rules(&self) -> [(&'static str, bool); 11] {
        [
...
mod tests {
    ...
    #[test]
    fn legacy_system_language_values_fall_back_to_chinese() {
        for value in ["\"System\"", "\"system\""] {
            let parsed: LanguageMode = serde_json::from_str(value).unwrap();
            assert_eq!(parsed, LanguageMode::ZhCn);
        }
    }

    #[test]
    fn language_values_serialize_to_public_locale_codes() {
        assert_eq!(
            serde_json::to_string(&LanguageMode::ZhCn).unwrap(),
            "\"zh-CN\""
        );
        assert_eq!(serde_json::to_string(&LanguageMode::En).unwrap(), "\"en\"");
    }
}
